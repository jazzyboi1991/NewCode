...
프로그램 최상위에서 이름 있는 기록 자료형을 선언할 수 있습니다. 모든 필드는
필수이며 생성자는 `be`를 사용하는 이름 있는 인자를 사용하므로 필드 순서는
중요하지 않습니다.
...
사용자 정의 기록은 다른 기록·목록 안에 중첩할 수 있고 `maybe`와 함께 사용할 수
있습니다. 누락·알 수 없는 필드는 `THINKLOGIC ERROR`, 필드 자료형 불일치는
`THINKTYPE ERROR`, 없는 필드 접근은 `INDEXCRIME`입니다.
